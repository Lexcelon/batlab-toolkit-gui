...
BatlabDebugDialog::BatlabDebugDialog(QWidget *parent,
  ...
  flashFirmwareButton =
      new QPushButton(tr("Download and flash firmware to Batlab"));
  ...
  connect(registerReadButton, &QPushButton::clicked, this,
          &BatlabDebugDialog::processRegisterReadClick);
  connect(registerWriteButton, &QPushButton::clicked, this,
          &BatlabDebugDialog::processRegisterWriteClick);
  connect(flashFirmwareButton, &QPushButton::clicked, this,
          &BatlabDebugDialog::processFirmwareFlashClick);
}

void BatlabDebugDialog::updateInfo(QVector<batlabStatusInfo> infos,
                                   QVector<QString> firmwareVersions) {
  ui->selectBatlabComboBox->clear();
  for (int i = 0; i < infos.size(); i++) {
    ui->selectBatlabComboBox->addItem(
        QString::number(infos.at(i).serialNumberComplete));
  }
...
BatlabDebugDialog::~BatlabDebugDialog() { delete ui; }

void BatlabDebugDialog::processRegisterReadClick() {
  emit registerReadRequested(ui->selectBatlabComboBox->currentText().toInt(),
                             registerReadNamespaceSpinbox->value(),
                             registerReadAddressSpinbox->value());
}

void BatlabDebugDialog::processRegisterWriteClick() {
  emit registerWriteRequested(ui->selectBatlabComboBox->currentText().toInt(),
                              registerWriteNamespaceSpinbox->value(),
                              registerWriteAddressSpinbox->value(),
                              registerWriteValueSpinbox->value());
}

void BatlabDebugDialog::processFirmwareFlashClick() {
  emit firmwareFlashRequested(ui->selectBatlabComboBox->currentText().toInt(),
                              flashFirmwareComboBox->currentText(),
                              "the existing version");
  done(QDialog::Accepted); // Firmware flashing makes things goofy so it is best
                           // that they reopen this dialog if they want later
}
