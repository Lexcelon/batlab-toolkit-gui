...
uint16_t Encoder::asIoff()
{
    return static_cast<uint16_t>(m_data * 128.0);
}

uint16_t Encoder::asSetpoint()
{
    return static_cast<uint16_t>(m_data * 128);
}

uint16_t Encoder::asMagDiv()
{
    return static_cast<uint16_t>(1 - log2(m_data));
}
...
{
    return static_cast<uint16_t>(static_cast<uint32_t>((m_data * 9.765625 / 4.096 / 6) * pow(2, 15)) >> 16);
}
