...
BatlabPacket::BatlabPacket() {
  m_startByte = static_cast<uchar>(0x00);
...
BatlabPacket::BatlabPacket(int batlabNamespace, int batlabRegister,
                           uchar payloadLowByte, uchar payloadHighByte) {
  m_startByte = static_cast<uchar>(0xAA);
  m_nameSpace = static_cast<uchar>(batlabNamespace);
...
BatlabPacket BatlabPacket::setStartByte(uchar startByte) {
  m_startByte = startByte;
  return *this;
}

uchar BatlabPacket::getNamespace() { return m_nameSpace; }
BatlabPacket BatlabPacket::setNamespace(uchar ns) {
  m_nameSpace = ns;
  return *this;
}

uchar BatlabPacket::getAddress() { return m_address; }
BatlabPacket BatlabPacket::setAddress(uchar address) {
  m_address = address;
  return *this;
}

uchar BatlabPacket::getPayloadLowByte() { return m_payloadLowByte; }
BatlabPacket BatlabPacket::setPayloadLowByte(uchar lowByte) {
  m_payloadLowByte = lowByte;
  return *this;
}

uchar BatlabPacket::getPayloadHighByte() { return m_payloadHighByte; }
BatlabPacket BatlabPacket::setPayloadHighByte(uchar highByte) {
  m_payloadHighByte = highByte;
  return *this;
}

int BatlabPacket::getWriteTimeout_ms() { return m_writeTimeout_ms; }
BatlabPacket BatlabPacket::setWriteTimeout_ms(int writeTimeout_ms) {
  m_writeTimeout_ms = writeTimeout_ms;
  return *this;
}
...
BatlabPacket BatlabPacket::setSleepAfterTransaction_ms(int ms) {
...
float BatlabPacket::asVcc() { return powf(2, 15) * 4.096f / getValue(); }

// Represents frequency data in Hz
float BatlabPacket::asFreq() { return getValue() * (10000.0f / 256.0f); }

// Represents register current to floating point Amps
float BatlabPacket::asIOff() { return getValue() / 128.0f; }

// Represents current setpoint as floating point Amps
float BatlabPacket::asSetPoint() { return getValue() / 128.0f; }

// Represents magdiv register as Ipp
float BatlabPacket::asMagdiv() { return 2.0f / powf(2, getValue()); }
...
float BatlabPacket::asTemperatureF(QVector<int> RList, QVector<int> BList) {
  return asTemperatureF(RList[static_cast<int>(getNamespace())],
                        BList[static_cast<int>(getNamespace())]);
}
...
float BatlabPacket::asTemperatureC(QVector<int> RList, QVector<int> BList) {
  return asTemperatureC(RList[static_cast<int>(getNamespace())],
                        BList[static_cast<int>(getNamespace())]);
}
...
//        if self.write == True:
//            logging.info("Wrote: Cell "+str(self.namespace)+", Addr
//            "+"{0:#4X}".format(self.addr & 0x7F))
//        else:
//            logging.info("Read: Cell "+str(self.namespace)+", Addr
//            "+"{0:#4X}".format(self.addr & 0x7F)+": "+str(self.data))

// def display(self):
//    """Prints out the basic info about the packet transaction ### charge
//    function.""" if(self.type == 'RESPONSE'):
//        if self.write == True:
//            print('Wrote: Cell '+str(self.namespace)+', Addr
//            '+"{0:#4X}".format(self.addr & 0x7F))
//        else:
// print('Read: Cell '+str(self.namespace)+', Addr '+"{0:#4X}".format(self.addr
// & 0x7F)+': '+str(self.data))
