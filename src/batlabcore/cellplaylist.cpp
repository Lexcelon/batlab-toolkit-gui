...
CellPlaylist::CellPlaylist() {
  ...
  playlistBatlabSettings.chargeCurrentSafetyCutoff =
      CHARGE_CURRENT_SAFETY_CUTOFF_DEFAULT;
  playlistBatlabSettings.dischargeCurrentSafetyCutoff =
...
QString CellPlaylist::toJson() {
  ...
  playlistJson[BATLAB_CELL_PLAYLIST_FILE_VERSION_FIELDSTR] =
  ...
  batlabSettingsJson[HIGH_VOLTAGE_CUTOFF_FIELDSTR] =
      playlistBatlabSettings.highVoltageCutoff;
  batlabSettingsJson[LOW_VOLTAGE_CUTOFF_FIELDSTR] =
      playlistBatlabSettings.lowVoltageCutoff;

  batlabSettingsJson[CHARGE_TEMP_CUTOFF_FIELDSTR] =
      playlistBatlabSettings.chargeTempCutoff;
  batlabSettingsJson[DISCHARGE_TEMP_CUTOFF_FIELDSTR] =
      playlistBatlabSettings.dischargeTempCutoff;

  batlabSettingsJson[CHARGE_CURRENT_SAFETY_CUTOFF_FIELDSTR] =
      playlistBatlabSettings.chargeCurrentSafetyCutoff;
  batlabSettingsJson[DISCHARGE_CURRENT_SAFETY_CUTOFF_FIELDSTR] =
      playlistBatlabSettings.dischargeCurrentSafetyCutoff;

  batlabSettingsJson[PRECHARGE_RATE_FIELDSTR] =
      playlistBatlabSettings.prechargeRate;
  batlabSettingsJson[CHARGE_RATE_FIELDSTR] = playlistBatlabSettings.chargeRate;
  batlabSettingsJson[DISCHARGE_RATE_FIELDSTR] =
      playlistBatlabSettings.dischargeRate;

  playlistJson[ACCEPTABLE_IMPEDANCE_THRESHOLD_FIELDSTR] =
  ...
  batlabSettingsJson[SINE_WAVE_FREQUENCY_FIELDSTR] =
      playlistBatlabSettings.sineWaveFrequency;
  batlabSettingsJson[SINE_WAVE_MAGNITUDE_FIELDSTR] =
      playlistBatlabSettings.sineWaveMagnitude;
...
bool CellPlaylist::load(QString filename) {
  ...
  const QVector<QString> requiredFields = {
      CELL_PLAYLIST_NAME_FIELDSTR, BATLAB_CELL_PLAYLIST_FILE_VERSION_FIELDSTR,
      CELL_CHEMISTRY_TYPE_FIELDSTR, CELL_NAMES_FIELDSTR,
      BATLAB_SETTINGS_FIELDSTR};

  for (int i = 0; i < requiredFields.length(); i++) {
    if (!jsonObject.contains(requiredFields[i])) {
      qWarning() << "Cell playlist file did not contain required field: "
                 << requiredFields[i];
      return false;
    }
  }

  QJsonObject batlabSettingsJsonObject =
      jsonObject[BATLAB_SETTINGS_FIELDSTR].toObject();

  setCellPlaylistName(jsonObject[CELL_PLAYLIST_NAME_FIELDSTR].toString());
  setBatlabCellPlaylistFileVersion(
      jsonObject[BATLAB_CELL_PLAYLIST_FILE_VERSION_FIELDSTR].toString());
  ...
  if (jsonObject.contains(NUM_MEASUREMENT_CYCLES_FIELDSTR)) {
    setNumMeasurementCycles(
        jsonObject[NUM_MEASUREMENT_CYCLES_FIELDSTR].toInt());
  }
  if (batlabSettingsJsonObject.contains(HIGH_VOLTAGE_CUTOFF_FIELDSTR)) {
    setHighVoltageCutoff(
        batlabSettingsJsonObject[HIGH_VOLTAGE_CUTOFF_FIELDSTR].toDouble());
  }
  if (batlabSettingsJsonObject.contains(LOW_VOLTAGE_CUTOFF_FIELDSTR)) {
    setLowVoltageCutoff(
        batlabSettingsJsonObject[LOW_VOLTAGE_CUTOFF_FIELDSTR].toDouble());
  }
  if (jsonObject.contains(STORAGE_DISCHARGE_FIELDSTR)) {
    setStorageDischarge(jsonObject[STORAGE_DISCHARGE_FIELDSTR].toBool());
  }
  if (jsonObject.contains(STORAGE_DISCHARGE_VOLTAGE_FIELDSTR)) {
    setStorageDischargeVoltage(
        jsonObject[STORAGE_DISCHARGE_VOLTAGE_FIELDSTR].toDouble());
  }
  if (jsonObject.contains(REST_PERIOD_FIELDSTR)) {
    setRestPeriod(jsonObject[REST_PERIOD_FIELDSTR].toInt());
  }
  if (batlabSettingsJsonObject.contains(CHARGE_TEMP_CUTOFF_FIELDSTR)) {
    setChargeTempCutoff(
        batlabSettingsJsonObject[CHARGE_TEMP_CUTOFF_FIELDSTR].toDouble());
  }
  if (batlabSettingsJsonObject.contains(DISCHARGE_TEMP_CUTOFF_FIELDSTR)) {
    setDischargeTempCutoff(
        batlabSettingsJsonObject[DISCHARGE_TEMP_CUTOFF_FIELDSTR].toDouble());
  }
  if (batlabSettingsJsonObject.contains(
          CHARGE_CURRENT_SAFETY_CUTOFF_FIELDSTR)) {
    setChargeCurrentSafetyCutoff(
        batlabSettingsJsonObject[CHARGE_CURRENT_SAFETY_CUTOFF_FIELDSTR]
            .toDouble());
  }
  if (batlabSettingsJsonObject.contains(
          DISCHARGE_CURRENT_SAFETY_CUTOFF_FIELDSTR)) {
    setDischargeCurrentSafetyCutoff(
        batlabSettingsJsonObject[DISCHARGE_CURRENT_SAFETY_CUTOFF_FIELDSTR]
            .toDouble());
  }
  if (batlabSettingsJsonObject.contains(PRECHARGE_RATE_FIELDSTR)) {
    setPrechargeRate(
        batlabSettingsJsonObject[PRECHARGE_RATE_FIELDSTR].toDouble());
  }
  if (batlabSettingsJsonObject.contains(CHARGE_RATE_FIELDSTR)) {
    setChargeRate(batlabSettingsJsonObject[CHARGE_RATE_FIELDSTR].toDouble());
  }
  if (batlabSettingsJsonObject.contains(DISCHARGE_RATE_FIELDSTR)) {
    setDischargeRate(
        batlabSettingsJsonObject[DISCHARGE_RATE_FIELDSTR].toDouble());
  }
  if (jsonObject.contains(ACCEPTABLE_IMPEDANCE_THRESHOLD_FIELDSTR)) {
    setAcceptableImpedanceThreshold(
        jsonObject[ACCEPTABLE_IMPEDANCE_THRESHOLD_FIELDSTR].toDouble());
  }
  if (jsonObject.contains(REPORTING_PERIOD_FIELDSTR)) {
    setReportingPeriod(jsonObject[REPORTING_PERIOD_FIELDSTR].toDouble());
  }
  if (jsonObject.contains(IMPEDANCE_REPORTING_PERIOD_FIELDSTR)) {
    setImpedanceReportingPeriod(
        jsonObject[IMPEDANCE_REPORTING_PERIOD_FIELDSTR].toDouble());
  }
  if (batlabSettingsJsonObject.contains(SINE_WAVE_FREQUENCY_FIELDSTR)) {
    setSineWaveFrequency(
        batlabSettingsJsonObject[SINE_WAVE_FREQUENCY_FIELDSTR].toDouble());
  }
  if (batlabSettingsJsonObject.contains(SINE_WAVE_MAGNITUDE_FIELDSTR)) {
  ...
  if (jsonObject.contains(CELL_LOG_TIMESTAMPS_FIELDSTR)) {
    setCellLogTimestamps(jsonObject[CELL_LOG_TIMESTAMPS_FIELDSTR].toBool());
  }
  if (jsonObject.contains(PLAYLIST_OUTPUT_DIRECTORY_FIELDSTR)) {
    setPlaylistOutputDirectory(
        jsonObject[PLAYLIST_OUTPUT_DIRECTORY_FIELDSTR].toString());
  }
  if (jsonObject.contains(PLAYLIST_SAVE_FILENAME_FIELDSTR)) {
    setPlaylistSaveFilename(
        jsonObject[PLAYLIST_SAVE_FILENAME_FIELDSTR].toString());
  }
...
QString CellPlaylist::getCellPlaylistName() { return cellPlaylistName; }
...
bool CellPlaylist::setCellChemistryType(QString str) {
  ...
  allowable_types << LIPO_CHEMISTRY_FIELDSTR
                  << IRON_PHOSPHATE_CHEMISTRY_FIELDSTR
                  << OTHER_CHEMISTRY_FIELDSTR;
...
QString CellPlaylist::getCellChemistryType() { return cellChemistryType; }
...
int CellPlaylist::getNumWarmupCycles() { return numWarmupCycles; }
...
int CellPlaylist::getNumMeasurementCycles() { return numMeasurementCycles; }
...
bool CellPlaylist::getStorageDischarge() { return storageDischarge; }

bool CellPlaylist::setStorageDischargeVoltage(double num) {
  if (STORAGE_DISCHARGE_VOLTAGE_MIN <= num &&
      num <= playlistBatlabSettings
                 .highVoltageCutoff) // Should set highVoltageCutoff first if it
                                     // is not the default
  {
...
bool CellPlaylist::setChargeCurrentSafetyCutoff(double num) {
  if (CHARGE_CURRENT_SAFETY_CUTOFF_MIN <= num &&
      num <= CHARGE_CURRENT_SAFETY_CUTOFF_MAX) {
...
bool CellPlaylist::setDischargeCurrentSafetyCutoff(double num) {
  if (DISCHARGE_CURRENT_SAFETY_CUTOFF_MIN <= num &&
      num <= DISCHARGE_CURRENT_SAFETY_CUTOFF_MAX) {
...
bool CellPlaylist::setPrechargeRate(double num) {
  if (PRECHARGE_RATE_MIN <= num &&
      num <= playlistBatlabSettings
                 .chargeCurrentSafetyCutoff) // Should set
                                             // chargeCurrentSafetyCutoff first
...
bool CellPlaylist::setDischargeRate(double num) {
  if (DISCHARGE_RATE_MIN <= num &&
      num <=
          playlistBatlabSettings
              .dischargeCurrentSafetyCutoff) // Should set
                                             // dischargeCurrentSafetyCutoff
                                             // first if it is not the default
  {
...
bool CellPlaylist::setAcceptableImpedanceThreshold(double num) {
  if (ACCEPTABLE_IMPEDANCE_THRESHOLD_MIN <= num &&
      num <= ACCEPTABLE_IMPEDANCE_THRESHOLD_MAX) {
...
double CellPlaylist::getReportingPeriod() { return reportingPeriod; }

bool CellPlaylist::setImpedanceReportingPeriod(double num) {
  if (IMPEDANCE_REPORTING_PERIOD_MIN <= num &&
      num <= IMPEDANCE_REPORTING_PERIOD_MAX) {
...
bool CellPlaylist::getIndividualCellLogs() { return individualCellLogs; }
...
bool CellPlaylist::getCellLogTimestamps() { return cellLogTimestamps; }
...
QString CellPlaylist::getPlaylistSaveFilename() { return playlistSaveFilename; }
...
QVector<QString> CellPlaylist::getCellNames() { return cellNames; }
