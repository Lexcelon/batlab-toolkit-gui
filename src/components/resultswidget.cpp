...
ResultsWidget::ResultsWidget(QVector<cellResultsStatusInfo> infos,
  ...
  connect(resetCellDataButton, &QPushButton::released, this,
          &ResultsWidget::resetCellData);

  // Create Select All Button
  selectAllButton = new QPushButton(tr("Select All"));
  resultsButtonLayout->addWidget(selectAllButton, 0, 5);
  selectAllButton->hide();
  connect(selectAllButton, &QPushButton::released, this,
          &ResultsWidget::selectAllRows);
  ...
  connect(selectCellsButton, &QPushButton::released, this,
          &ResultsWidget::showSelectCells);
  ...
  resultsTableWidget->setHorizontalHeaderItem(
      1, new QTableWidgetItem(tr("Capacity (C)")));
  resultsTableWidget->setHorizontalHeaderItem(
      2, new QTableWidgetItem(tr("Capacity Range (C)")));
  resultsTableWidget->setHorizontalHeaderItem(
      3, new QTableWidgetItem(tr("Impedance (Ohm)")));
  resultsTableWidget->setHorizontalHeaderItem(
      4, new QTableWidgetItem(tr("Avg Voltage (V)")));
  resultsTableWidget->setHorizontalHeaderItem(
      5, new QTableWidgetItem(tr("Avg Current (A)")));
  ...
  for (int i = 0; i < infos.length(); i++) {
    ...
    // Results complete for this cell
      ...
      resultsTableWidget->setItem(i, 1,
                                  new QTableWidgetItem(QString::number(
                                      static_cast<double>(infos[i].capacity))));
      resultsTableWidget->setItem(
          i, 2,
          new QTableWidgetItem(
              QString::number(static_cast<double>(infos[i].capacityRange))));
      resultsTableWidget->setItem(
          i, 3,
          new QTableWidgetItem(
...
void ResultsWidget::showSelectCells() {
  ...
  disconnect(selectCellsButton, &QAbstractButton::released, this,
             &ResultsWidget::showSelectCells);
  connect(selectCellsButton, &QAbstractButton::released, this,
          &ResultsWidget::hideSelectCells);
...
void ResultsWidget::resetCellData() {
  ...
  for (int i = 0; i < infos.length(); i++) {
    if (i < checkBoxItems.length() &&
        checkBoxItems[i]->checkState() == Qt::Checked) {
  ...
  if (cellsToReset.length() > 0) {
    ...
    reply = QMessageBox::question(
        this, "Reset Selected Cell Data",
        "Are you sure you want to reset the selected cell data?",
    ...
    QMessageBox::StandardButton reply;
    reply = QMessageBox::information(this, "No Cells Selected",
                                     "You did not select any cells.",
                                     QMessageBox::Ok);
  }
}
